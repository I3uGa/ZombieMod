...
int64 FASTCALL Detour_CBaseEntity_TakeDamageOld(CBaseEntity* pThis, CTakeDamageInfo* pInfo, CTakeDamageResult* pResult)
{
	...
	if (pResult->m_flDamageDealt > 0 && !pResult->m_bWasDamageSuppressed && g_cvarZMEnable.Get() && pThis->IsPawn())
		ZM_OnPlayerTakeDamage(reinterpret_cast<CCSPlayerPawn*>(pThis), pInfo, pResult->m_flDamageDealt);
